...
from fmCrawler.preprocessor import preprocessor

# useful for handling different item types with a single interface
from itemadapter import ItemAdapter

class FmcrawlerPipeline:
    crawled_count = 0
    duplicate_count = 0
    new_comments = 0
    is_first = True
    ...
    def process_item(self, item, spider):
        if self.is_first: # argv를 받으려면 여기서 해야만 함 
            log_util = LogUtil('fmCrawler' + '-' + str(spider.sub_id))
        ...
        self.fm_index=f"crawling-fmkorea-{index_date}"

        if self.es.indices.exists(index = self.fm_index):
            pass
        else:
            self.es.indices.create(index = self.fm_index,body=self.fm_mapping)
        ...
        if res['hits']['total']['value'] == 0:             
            self.es.index(index=self.fm_index, body=doc, pipeline='timestamp')
        else:      
            self.es.update_by_query(pipeline='timestamp',index=self.fm_index,body={
                "script":{
    ...
    def process_embedding(self, doc): 
        ...
        if len(doc['title'].split()) >= 4:
            ...
            # 5. 저장할 포멧으로 변경 
            title_doc = dict()
            title_doc['id'] = 'fmt-'+ str(doc['post_id']) # tstory title
        ...
        for comment in doc['comment']: 
            ...
            for comment_text in comment_texts: 
                ...
                if len(comment_text.split()) >= 4:
                    ...
                    # 7. 저장할 포멧으로 변경 
                    comment_doc = dict()
                    comment_doc['id'] = f"fmc-{doc['post_id']}-{comment['id']}"   # naver cafe comment
