...
from youtubeCrawler.preprocessor import preprocessor
...
class YoutubeCrawlerPipeline(object):
    ...
    def process_item(self, item, spider):
        if self.is_first: # argv를 받으려면 여기서 해야만 함 
            log_util = LogUtil('youtubeCrawler' + '-' + str(1)) #spider.sub_id
        ...
        self.youtube_index = f"crawling-youtube-{index_date}"
        
        if self.es.indices.exists(index = self.youtube_index):
            pass
        else:
            self.es.indices.create(index = self.youtube_index,body=self.youtube_mapping)
        ...
        # youtube_info가 기존에 저장되어 있으면 저장안함 
        res = self.es.search(
                    index = self.youtube_index,
                    body = 
                    {
	                    "query": {
	                        "bool": {
        ...
        if res['hits']['total']['value'] == 0:             
            self.es.index(index=self.youtube_index, body=doc, pipeline='timestamp')
        else:      
            self.es.update_by_query(pipeline='timestamp',index=self.youtube_index,body={
                "script":{
    ...
    def process_embedding(self, doc): 
        ...
        if doc['title'] is None:
            doc['title'] = ''
        self.log.debug ('after delete special word: '+str(doc['title']))
        # doc['title'] = self.spacing(doc['title']) 띄어쓰기 라이브러리 검토 필요 
        ...
        if len(doc['title'].split()) >= 4:
            ...
            # 5. 저장할 포멧으로 변경 
            title_doc = dict()
            title_doc['id'] = 'ytt-'+ str(doc['post_id']) # tstory title
            title_doc['url'] = doc['url']
            title_doc['date'] = doc['date']
            title_doc['keywords'] = doc['keyword']
        ...
        for comment in doc['comment']: 
            ...
            for comment_text in comment_texts: 
                ...
                if len(comment_text.split()) >= 4:
                    ...
                    # 7. 저장할 포멧으로 변경 
                    comment_doc = dict()
                    comment_doc['id'] = f"ytc-{doc['post_id']}-{comment['id']}"   # naver cafe comment
