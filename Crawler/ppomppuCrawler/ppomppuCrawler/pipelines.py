...
from ppomppuCrawler.preprocessor import preprocessor

# useful for handling different item types with a single interface
from itemadapter import ItemAdapter


class PpomppucrawlerPipeline:
    crawled_count = 0
    duplicate_count = 0
    new_comments = 0
    is_first = True
    ...
    def process_item(self, item, spider):
        if self.is_first: # argv를 받으려면 여기서 해야만 함 
            log_util = LogUtil('ppomppuCrawler' + '-' + str(spider.sub_id))
        ...
        self.ppomppu_index = f"crawling-ppomppu-{index_date}"

        if self.es.indices.exists(index = self.ppomppu_index):
            pass
        else:
            self.es.indices.create(index = self.ppomppu_index,body=self.ppomppu_mapping)
        ...
        if res['hits']['total']['value'] == 0:             
            self.es.index(index=self.ppomppu_index, body=doc, pipeline='timestamp')
        else:      
            self.es.update_by_query(pipeline='timestamp',index=self.ppomppu_index,body={
                "script":{
    ...
    def process_embedding(self, doc): 
        ...
        if len(doc['title'].split()) >= 4:
            ...
            # 5. 저장할 포멧으로 변경 
            title_doc = dict()
            title_doc['id'] = 'dct-'+ str(doc['post_id']) # tstory title
            ...
            title_doc['num_dislike'] = doc['dislike']
        ...
        for comment in doc['comment']: 
            ...
            for comment_text in comment_texts: 
                ...
                if len(comment_text.split()) >= 4:
                    ...
                    # 7. 저장할 포멧으로 변경 
                    comment_doc = dict()
                    comment_doc['id'] = f"dcc-{doc['post_id']}-{comment['id']}"   # naver cafe comment
                    ...
                    comment_doc['num_dislike'] = doc['dislike']
