...
from tstoryCrawler.preprocessor import preprocessor


class TstorycrawlerPipeline:
    crawled_count = 0
    duplicate_count = 0
    new_comments = 0
    is_first = True
    ...
    def process_item(self, item, spider):
        ...
        if self.is_first: # argv를 받으려면 여기서 해야만 함 
            log_util = LogUtil('tstoryCrawler' + '-' + str(spider.sub_id))
        ...
        self.tstory_index = f"crawling-tstory-{index_date}"

        if self.es.indices.exists(index = self.tstory_index):
            pass
        else:
            self.es.indices.create(index = self.tstory_index,body=self.tstory_mapping)
        ...
        if res['hits']['total']['value'] == 0:             
            self.es.index(index=self.tstory_index, body=doc, pipeline='timestamp')
        else:      
            self.es.update_by_query(pipeline='timestamp',index=self.tstory_index,body={
                "script":{
    ...
    def process_embedding(self, doc): 
        ...
        if len(doc['title'].split()) >= 4:
            ...
            # 5. 저장할 포멧으로 변경 
            title_doc = dict()
            title_doc['id'] = 'tst-'+ str(doc['blog_id']) + '-' + doc['entry_id'] # tstory title
        ...
        for comment in doc['comment']: 
            ...
            for comment_text in comment_texts: 
                ...
                if len(comment_text.split()) >= 4:
                    ...
                    # 7. 저장할 포멧으로 변경 
                    comment_doc = dict()
                    comment_doc['id'] = 'tsc-' + str(doc['blog_id']) + '-' + doc['entry_id'] + '-' + str(comment['id']) # naver cafe comment
