...
from dcCrawler.preprocessor import preprocessor

# useful for handling different item types with a single interface
from itemadapter import ItemAdapter


class DccrawlerPipeline:
    crawled_count = 0
    duplicate_count = 0
    new_comments = 0
    is_first = True
    ...
    def process_item(self, item, spider):
        if self.is_first: # argv를 받으려면 여기서 해야만 함 
            log_util = LogUtil('dcCrawler' + '-' + str(spider.sub_id))
        ...
        self.dc_index=f"crawling-dcinside-{index_date}"

        if self.es.indices.exists(index = self.dc_index):
            pass
        else:
            self.es.indices.create(index = self.dc_index,body=self.dc_mapping)
        ...
        if res['hits']['total']['value'] == 0:             
            self.es.index(index=self.dc_index, body=doc, pipeline='timestamp')
        else:      
            self.es.update_by_query(pipeline='timestamp',index=self.dc_index,body={
                "script":{
