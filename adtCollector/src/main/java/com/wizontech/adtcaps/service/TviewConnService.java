...
@Slf4j
@EnableScheduling
@Service("TviewConnService")
public class TviewConnService implements CollectDao {

    @Autowired
    private FileUtil fileUtil;

    @Autowired
    Map<String, String> tviewConnMap;

    @Override
    public void writeJsonToFile(String jsonStr) {

        fileUtil.writeFile("tview_conn", jsonStr);
    ...
    public ResponseEntity<String> getData(String bodyData) {
        ...
        Gson gson = new Gson();
        Map<String, String> keyMap = tviewConnMap;
            ...
            for (JsonElement dataElement : dataArr) {
                ...
                try {
                    ...
                    try {
                        ...
                        // 테스트 출력
                        log.info("==> tviewConn // currObjTime :: " + currObjTime);
                    } catch (Exception e) {
                        log.error(e.toString());
                    }
                    ...
                    // 장애|복구 판단 보류
                    // if (tviewConnMap.containsKey(key)) {
                    // prevObj = gson.fromJson(tviewConnMap.get(key), JsonObject.class);
                    // }
